...
from dataset.Musk_dataset import MUSK_Parser
...
def one_proc(eps):
    ...
    for attr in [WA]:#属性の決定方法
        for attr_size in [10,20,30,40,50]:
            for n_class in range(2, 5):
                for train_type in [WA]:
                    for test_type in [WA]:
    ...
    Path(f"result_Musk/{names[attr]}/{names[train_type]}_{names[test_type]}/").mkdir(parents=True, exist_ok=True)
    with open(f"result_Musk/{names[attr]}/{names[train_type]}_{names[test_type]}/combination_all.csv", 'a') as ofile:
