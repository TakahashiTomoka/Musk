...
def one_proc(step1=RANDOM, train_type=NO_NOISE, test_type=NO_NOISE, attr_size=3, n_class=5, eps_begin=3, eps_end=5):
    ...
    while eps <= eps_end+0.05:
        ...
        Path(f"result_Musk/{names[step1]}/{names[train_type]}_{names[test_type]}").mkdir(parents=True, exist_ok=True)
        with open(f"result_Musk/{names[step1]}/{names[train_type]}_{names[test_type]}/{attr_size}_C{n_class}.csv", 'a') as ofile:
